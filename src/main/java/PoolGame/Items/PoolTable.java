...
public class PoolTable implements Drawable {
     ...
     * @param pockets the config pocket
     */
    ...
    /**
     * Make the hard copy of pooltable and other attribute
     * @param table the table want to be copied
     */
    public PoolTable(PoolTable table) {
        this.balls = new ArrayList<Ball>();
        this.pockets = new ArrayList<Pocket>();
        this.colour = table.getColour();
        this.colourName = table.getColour().toString();
        this.friction = table.getFriction();
        this.shape = new Rectangle(table.getDimX(), table.getDimY(), this.colour);

        for (Pocket i: table.getPockets()) {
            this.pockets.add(i);
        ...
        this.dim = new long[2];
        this.dim[0] = table.getDimX();
        this.dim[1] = table.getDimY();
    }
    ...
    private void init(String colourName, double friction, long dimX, long dimY, PocketsConfig pocketsConfig) {
        ...
        //Added
        this.pocketsInit(pocketsConfig);
    }

    /**
     * Make the pockets with given pocket config
     * @param config
     */
    private void pocketsInit(PocketsConfig config) {
        this.pockets = new ArrayList<Pocket>();
        for (PocketConfig i: config.getPocketConfigs()) {
            double x = i.getPosition().getX();
            double y = i.getPosition().getY();
            double radius = i.getRadius();
            Pocket pocket = new Pocket(x, y);
